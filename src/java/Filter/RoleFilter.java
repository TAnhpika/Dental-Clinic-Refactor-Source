...
public class RoleFilter implements Filter {
    ...
    public void doFilter(ServletRequest req, ServletResponse res, FilterChain chain)
        ...
        if (isPublicPage(path)) {
            if (DEBUG)
                System.out.println("✅ Public page: " + path);
        ...
        if (hasAccess(userRole, path)) {
            if (DEBUG)
                System.out.println("✅ Access granted for " + userRole + " to " + path);
            chain.doFilter(req, res);
        } else {
            if (DEBUG)
                System.out.println("❌ Access denied for " + userRole + " to " + path);
    ...
    private void redirectToLogin(HttpServletRequest request, HttpServletResponse response)
            throws IOException {
        String loginUrl = request.getContextPath() + "/jsp/auth/login.jsp";
