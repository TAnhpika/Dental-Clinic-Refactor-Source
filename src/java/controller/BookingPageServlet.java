...
public class BookingPageServlet extends HttpServlet {
    ...
    /*
        Chức năng chính: Xử lý các yêu cầu liên quan đến việc đặt lịch khám bệnh, bao gồm:
    ...
    */
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (serviceIdStr != null && !serviceIdStr.isEmpty()) {
                try {
                    ...
                    if (selectedService == null || selectedService.getServiceId() != serviceId) {
                        ...
                        System.out.println("🎯 Service được chọn với giá cố định: " + selectedService.getServiceName() + " - 50,000 VNĐ");
                    }
            ...
            // Lấy danh sách bác sĩ - CHỈ HIỂN THỊ BÁC SĨ ĐÚNG CHUYÊN KHOA CỦA DỊCH VỤ ĐÃ CHỌN
            ...
            if (selectedService != null) {
                ...
                if (specialtyNameForFilter == null && selectedService.getCategory() != null && !selectedService.getCategory().isEmpty())
            ...
            if (specialtyNameForFilter != null && !specialtyNameForFilter.isEmpty()) {
                ...
                System.out.println("🔍 Filter bác sĩ theo chuyên khoa: " + specialtyNameForFilter + (keyword != null && !keyword.isEmpty() ? ", từ khóa: " + keyword : "") + " -> " + doctors.size() + " bác sĩ");
                // Khi đã chọn dịch vụ: CHỈ trả về bác sĩ đúng chuyên khoa, không fallback tất cả
            ...
            // Lấy danh sách dịch vụ để hiển thị trong popup chọn dịch vụ - CHỈ LOAD NẾU CHƯA CÓ DỊCH VỤ ĐÃ CHỌN
            // Nếu đã có selectedService, không cần load lại để tránh người dùng chọn lại
            if (selectedService == null) {
                try {
                    ServicePriceDAO servicePriceDAO = new ServicePriceDAO();
                    List<Service> services = servicePriceDAO.getAllServicesWithFixedPrice();                               // xử lý bên servicePriceDAO
            ...
            // Lấy danh sách ngày làm việc của bác sĩ (nếu có doctorId)                                                         // render ngày làm việc cảu bác sĩ theo  getWorkDatesByDoctorId 
            ...
            request.getRequestDispatcher("/jsp/patient/user_datlich.jsp").forward(request, response);  // forward ra ở đây 
...
//===================================================================================
    //doPost: Xử lý yêu cầu HTTP POST để thực hiện đặt lịch hoặc tạo thông tin người thân.
    @Override
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            StringBuilder paymentUrlBuilder = new StringBuilder();                                                                           // khởi tạo đối tượng string builder với object để xây dựng URL. StringBuilder được sử dụng vì nó hiệu quả hơn khi nối chuỗi so với String.
